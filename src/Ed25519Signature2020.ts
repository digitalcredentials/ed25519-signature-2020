...
import {
  ...
  Verifier
} from '@digitalcredentials/linked-data-integrity'
...
export class Ed25519Signature2020 extends LinkedDataSignature {
  ...
  static CONTEXT_URL: string = SUITE_CONTEXT_URL
  static CONTEXT: any = suiteContext2020.contexts.get(SUITE_CONTEXT_URL)
  ...
  constructor({
    ...
    super({
      ...
      KeyPairClass: Ed25519VerificationKey2020,
  ...
  async verifySignature({
    ...
    let { verifier } = this
    if (!verifier) {
      const key = await this.KeyPairClass.from(verificationMethod)
  ...
  async assertVerificationMethod({
    ...
    if (verificationMethod.type === 'Ed25519VerificationKey2020') {
      ...
      throw new Error(
        `Unsupported key type "${verificationMethod.type as string}".`
      )
    }
    if (
      !_includesContext({
